...
__global__ void insertKernel(Heap *heap, int *items, int arraySize, int nodeSize) {
    int batchNeed = (arraySize + nodeSize - 1) / nodeSize;
    // No more blocks needed
    if(blockIdx.x >= batchNeed) return;
    // Each thread in block inserts its block to the heap in parallel
    heap->insertion(items + blockIdx.x * nodeSize, nodeSize, 0);
    __syncthreads();
}

__global__ void insertKernelDifferentSizes(Heap *heap, int *items, int arraySize, int nodeSize) {
...
void testGpuHeap(int *h_tItems, int numThreadsPerBlock, int nodeSize, int arraySize){
    ...
    // call the kernel 
    startTime(&timer);
    insertKernelDifferentSizes<<<numBlocks, numThreadsPerBlock, smemSize>>>(d_heap, heapItems, arraySize, nodeSize);
