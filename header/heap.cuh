...
class Heap {
    public:
        ...
        // prints the array heapItems 
        void printHeapTree() {
            for (int i = 0; i< 1000; i++) {
                printf("%d ", heapItems[i]);
            }
        }
        ...
        // Returns the number of nodes (batches) in the heap.
        int nodeCount() {
            int bcount;
        ...
        // Returns the total number of items in the heap (on the device).
        __device__ int getItemCount() {
            changeStatus(&status[0], AVAIL, INUSE);
            int itemCount = *partialBufferSize + * currNumNodes * nodeSize;
        ...
        __device__ void deleteUpdate(int smemOffset) {
            ...
            imergePath(sMergedItems, sMergedItems + nodeSize,
            ...
            while (1) {
                ...
                if (deleteType == 0) { // no children
                    ...
                    imergePath(sMergedItems, sMergedItems + nodeSize,
                               heapItems + currentIdx * nodeSize, heapItems + leftIdx * nodeSize,
                               nodeSize, smemOffset);
                    __syncthreads();
                ...
                batchCopy(sMergedItems + nodeSize,
                          heapItems + leftIdx * nodeSize,
                          nodeSize);
                batchCopy(sMergedItems + 2 * nodeSize,
                          heapItems + rightIdx * nodeSize,
                          nodeSize);
                ...
                imergePath(sMergedItems + nodeSize, sMergedItems + 2 * nodeSize,
                           sMergedItems + nodeSize, heapItems + largerIdx * nodeSize,
                           nodeSize, smemOffset);
                __syncthreads();
                ...
                if (sMergedItems[0] >= sMergedItems[2 * nodeSize - 1]) {
                    batchCopy(heapItems + currentIdx * nodeSize,
                              sMergedItems + nodeSize,
                              nodeSize);
                }
                    ...
                    batchCopy(heapItems + currentIdx * nodeSize, 
                              sMergedItems,
                              nodeSize);
                    batchCopy(heapItems + smallerIdx * nodeSize, 
                              sMergedItems + nodeSize,
                              nodeSize);
                    if (threadIdx.x == 0) {
                        changeStatus(&status[currentIdx], INUSE, AVAIL);
                    ...
                    imergePath(sMergedItems, sMergedItems + nodeSize,
                               heapItems + currentIdx * nodeSize, sMergedItems,
                               nodeSize, smemOffset);
                }
                __syncthreads();
        ...
        __device__ void insertion(int *items, int size, int smemOffset) {
            ...
            extern __shared__ int s[];
            int *sMergedItems1 = (int *)&items[0];
            int *sMergedItems2 = (int *)&s[smemOffset];
            smemOffset += sizeof(int) * nodeSize / sizeof(int);
            ...
            smemOffset += sizeof(int) * 2 * nodeSize / sizeof(int);
            int *tmpIdx = (int *)&s[smemOffset - 1];
            ...
            if (threadIdx.x == 0) {
                changeStatus(&status[0], AVAIL, INUSE);
//                printf("%d %d\n", *globalBenefit, *currNumNodes);
            }
            __syncthreads();

            /* start handling partial batch */
            // Case 1: the heap has no full batch
            // TODO current not support size > nodeSize, app should handle this
            ...
            while(currentIdx != targetIdx) {
                ...
                if (sMergedItems1[nodeSize - 1] <= sMergedItems2[0]) {
                    ...
                    __syncthreads();
                    imergePath(sMergedItems1, sMergedItems2,
                            heapItems + currentIdx * nodeSize, sMergedItems1,
                            nodeSize, smemOffset);
                    __syncthreads();
                }
